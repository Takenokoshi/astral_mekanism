...
public class PhotonAnnihilationPlanePart extends AEBasePart implements IGridTickable {
    ...
    /**
     * Enchantments found on the plane when it was placed will be used to enchant
     * the fake tool used for picking up
     * blocks.
     */
    @Nullable
    private Map<Enchantment, Integer> enchantments;

    // Allows annihilation planes to stop pickup and instead go into a continuous
    // generation mode
    private ContinuousGeneration continuousGeneration;
    private int continuousGenerationTicks;

    public PhotonAnnihilationPlanePart(IPartItem<?> partItem) {
        super(partItem);
    ...
    public void addToWorld() {
        ...
        if (host.getBlockPos().getY() + 1 >= buildHeight && getSide() == Direction.UP) {
            continuousGeneration = new ContinuousGeneration(
                    ...
                    200,
                    100);
        } else if (host.getBlockPos().getY() <= host.getLevel().getMinBuildHeight() && getSide() == Direction.DOWN) {
            continuousGeneration = new ContinuousGeneration(
                    AEItemKey.of(AMEItems.XP_CRYSTAL),
                    1,
                    400);
        }
    }
    ...
    public void onUpdateShape(Direction side) {
        ...
        if (side.equals(ourSide)) {
            ...
            // Changes perpendicular to our side may change the connected plane model to
            // change
    ...
    public TickRateModulation tickingRequest(IGridNode node, int ticksSinceLastCall) {
        ...
        if (isActive() && continuousGeneration != null) {
            ...
            if (continuousGenerationTicks >= continuousGeneration.ticks) {
                ...
                insertIntoGrid(continuousGeneration.what, amount * continuousGeneration.amount, Actionable.MODULATE);
