package astral_mekanism.enumexpansion;

import mekanism.api.text.APILang;

public class AMEAPILang {
    public static APILang UPGRADE_COBBLESTONE_SUPPLY;
    public static APILang UPGRADE_WATER_SUPPLY;
    public static APILang UPGRADE_XP;
    public static APILang UPGRADE_RADIOACTIVE_SEALING;
    public static APILang UPGRADE_AIR_INTAKE;
    public static APILang UPGRADE_HYPER_SPEED;
    public static APILang UPGRADE_STARDUST_SPEED;
    public static APILang UPGRADE_ADVANCED_STONE_GENERATOR;
    public static APILang UPGRADE_TANK_CAPACITY;
    public static APILang UPGRADE_ADVANCED_TANK_CAPACITY;

    public static APILang UPGRADE_COBBLESTONE_SUPPLY_DESCRIPTION;
    public static APILang UPGRADE_WATER_SUPPLY_DESCRIPTION;
    public static APILang UPGRADE_XP_DESCRIPTION;
    public static APILang UPGRADE_RADIOACTIVE_SEALING_DESCRIPTION;
    public static APILang UPGRADE_AIR_INTAKE_DESCRIPTION;
    public static APILang UPGRADE_HYPER_SPEED_DESCRIPTION;
    public static APILang UPGRADE_STARDUST_SPEED_DESCRIPTION;
    public static APILang UPGRADE_ADVANCED_STONE_GENERATOR_DESCRIPTION;
    public static APILang UPGRADE_TANK_CAPACITY_DESCRIPTION;
    public static APILang UPGRADE_ADVANCED_TANK_CAPACITY_DESCRIPTION;
}
