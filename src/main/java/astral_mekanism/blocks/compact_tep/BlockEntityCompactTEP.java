package astral_mekanism.blocks.compact_tep;
...
public class BlockEntityCompactTEP extends TileEntityRecipeMachine<FluidToFluidRecipe>
	...
	public BlockEntityCompactTEP(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
		...
		if (fluidConfig != null) {
			...
			fluidConfig.addSlotInfo(DataTypeEx.INPUT1_OUTPUT2, new FluidSlotInfo(true, false, inputTank));
	...
	protected void onUpdateServer() {
		...
		tempMultiplier = Math.floor((Math.min(MAX_MULTIPLIER_TEMP, heatCapacitor.getTemperature())
				- HeatAPI.AMBIENT_TEMP) * MekanismConfig.general.evaporationTempMultiplier.get());
		...
		if (ejectorComponent.isEjecting(configComponent.getConfig(TransmissionType.FLUID),
			...
			FluidUtils.emit(configComponent.getConfig(TransmissionType.FLUID)
					.getSidesForData(DataTypeEx.INPUT1_OUTPUT2), outputTank, this, 2147483647);
		}
	}
	...
	public BlockEntityCompactTEP getMultiblock() {
		return this;
	}
