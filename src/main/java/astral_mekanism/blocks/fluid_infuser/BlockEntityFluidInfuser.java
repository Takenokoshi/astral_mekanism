package astral_mekanism.blocks.fluid_infuser;
...
public class BlockEntityFluidInfuser extends TileEntityRecipeMachine<FluidFluidToFluidRecipe>
	...
	BasicFluidTank inputTankA;
	BasicFluidTank inputTankB;
	BasicFluidTank outputTank;
	MachineEnergyContainer<BlockEntityFluidInfuser> energyContainer;
	...
	public BlockEntityFluidInfuser(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
		...
		if (fluidConfig != null) {
			...
			fluidConfig.addSlotInfo(DataTypeEx.INPUT1_OUTPUT, new FluidSlotInfo(true, false, inputTankA));
			fluidConfig.addSlotInfo(DataTypeEx.INPUT2_OUTPUT, new FluidSlotInfo(true, false, inputTankB));
	...
	protected void onUpdateServer() {
		...
		ConfigInfo fluidConfig = configComponent.getConfig(TransmissionType.FLUID);
		if (ejectorComponent.isEjecting(fluidConfig, TransmissionType.FLUID)) {
			FluidUtils.emit(fluidConfig.getSidesForData(DataTypeEx.INPUT1_OUTPUT), outputTank, this,
					200000);
			FluidUtils.emit(fluidConfig.getSidesForData(DataTypeEx.INPUT2_OUTPUT), outputTank, this,
					200000);
		}
	...
	public @NotNull CachedRecipe<FluidFluidToFluidRecipe> createNewCachedRecipe(
		...
		MachineEnergyContainer<BlockEntityFluidInfuser> energyContainer = this.energyContainer;
	...
	public MachineEnergyContainer<BlockEntityFluidInfuser> getEnergyContainer() {
