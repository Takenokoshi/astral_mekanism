...
import astral_mekanism.jei.MekJEIRecipeTypes;
...
public class GuiFluidInfuser
		extends GuiConfigurableTile<BlockEntityFluidInfuser, MekanismTileContainer<BlockEntityFluidInfuser>> {

	public GuiFluidInfuser(MekanismTileContainer<BlockEntityFluidInfuser> container, Inventory inv,
			Component title) {
	...
	protected void addGuiElements() {
		...
		addRenderableWidget(new GuiFluidGauge(() -> tile.inputTankA,
				...
				.warning(WarningType.NO_MATCHING_RECIPE, tile.getWarningCheck(
						BlockEntityFluidInfuser.NOT_ENOUGH_FLUIDA_INPUT_ERROR));
		addRenderableWidget(new GuiFluidGauge(() -> tile.inputTankB,
				() -> List.of(tile.inputTankA, tile.inputTankB, tile.outputTank), GaugeType.STANDARD,
				this, 133, 13))
				.warning(WarningType.NO_MATCHING_RECIPE, tile.getWarningCheck(
						BlockEntityFluidInfuser.NOT_ENOUGH_FLUIDB_INPUT_ERROR));
		...
		addRenderableWidget(new GuiProgress(tile::getActive, ProgressType.SMALL_RIGHT, this, 47, 39))
				...
				.jeiCategories(MekJEIRecipeTypes.FLUID_INFUSER_RECIPE);
