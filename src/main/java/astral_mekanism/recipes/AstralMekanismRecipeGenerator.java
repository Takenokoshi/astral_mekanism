...
@Mod.EventBusSubscriber(modid = AstralMekanismID.MODID, value = Dist.DEDICATED_SERVER, bus = Mod.EventBusSubscriber.Bus.FORGE)
public class AstralMekanismRecipeGenerator {
    ...
    public static void onReload(AddReloadListenerEvent event) {
        event.addListener(new SimplePreparableReloadListener<Object>() {
            ...
            @Override
            protected void apply(Object obj, ResourceManager manager, ProfilerFiller profiler) {
                RecipeManager recipeManager = ServerLifecycleHooks.getCurrentServer()
                        .getRecipeManager();
                ...
                IChemicalStackIngredientCreator<Gas, GasStack, GasStackIngredient> gasCreator = IngredientCreatorAccess
                        .gas();

                Collection<Recipe<?>> beforeRecipes = recipeManager.getRecipes();
                List<Recipe<?>> forPredicate = beforeRecipes.stream()
                        .filter(r -> r.getId().getNamespace().equals(AstralMekanismID.MODID)).toList();
                Predicate<ResourceLocation> idPredicate = id -> forPredicate.stream()
                    ...
                    if (recipe instanceof ChargerRecipe chargerRecipe) {
                        ...
                        afterRecipes.add(new MekanicalChagerIRecipe(id,
                                itemCreator.createMulti(
                                        ingredients.stream().map(itemCreator::from)
                        ...
                        if (idPredicate.test(id)) {
                            ...
                            && transformRecipe.circumstance.isFluid()) {
                        afterRecipes.add(recipe);
                        ResourceLocation id = AstralMekanismID.reRL(recipe.getId(), "ae2_mekanism_styled/");
                        if (idPredicate.test(id)) {
                            continue;
                            ...
                            afterRecipes.add(new FormulizedSawingIRecipe(id,
                                    itemCreator.createMulti(sawmillIRecipe.getInput().getRepresentations().stream()
                                            .map(stack -> itemCreator.from(
                                                    stack.copyWithCount(
                                                            stack.getCount() * (int) Math.ceil(1d / chance))))
                                            .toArray(ItemStackIngredient[]::new)),
                        ...
                        afterRecipes.add(new CompressingAMIRecipe(id, compressingIRecipe.getItemInput(),
                                gasCreator.createMulti(compressingIRecipe.getChemicalInput().getRepresentations()
                                        .stream()
                ...
                recipeManager.replaceRecipes(afterRecipes);
            }
        });
    }
}